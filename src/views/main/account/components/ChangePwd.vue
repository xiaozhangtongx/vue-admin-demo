  ...
  <div>
    <el-dialog title="修改密码" :visible.sync="changePwdDialogVisible" width="50vh"
      ...
      <el-form :model="newPwdForm" :rules="newPwdFormRules" ref="newPwdFormRef" label-width="30%"
        style="width:96%">
        <!-- 新密码 -->
        <el-form-item label="新密码：" prop="upward">
          <el-input show-password type="password" v-model="newPwdForm.upward"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button style="margin-left:20px" type="primary" @click="changePwd">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 展示修改密码对话框
    showchangePwd(gestInfo) {
      this.changePwdDialogVisible = true
      this.oldpwd = gestInfo.upward
      this.newPwdForm.uid = gestInfo.uid
    },
    // 关闭对话框
